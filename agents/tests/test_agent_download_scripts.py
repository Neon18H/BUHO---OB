...
class AgentDownloadScriptTests(TestCase):
    ...
    def test_windows_installer_uses_schtasks_system_onstart_and_runner_logging(self):
        ...
        self.assertIn('if (-not $procs -or $procs.Count -eq 0) {', script)
        self.assertIn('Smoke test warning:', script)
        self.assertIn('WARNING: smoke test lanzó excepción (no fatal).', script)
        self.assertIn('Manual repair', script)
